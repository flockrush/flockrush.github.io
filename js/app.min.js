...
}; var initContactForm = function ($, wrapperSelector, triggerSelector) { var $wrapper = $(wrapperSelector); var $triggers = $(triggerSelector); if ($wrapper.length > 0 && $triggers.length > 0) { $triggers.click(function () { if ($wrapper.hasClass('opened')) { $wrapper.removeClass('opened') } else { $wrapper.addClass('opened'); $wrapper.find('.close').one('click', function () { $wrapper.removeClass('opened') }) } }) } }; var initHeadroom = function ($, wrapperClass, options) { var main = $(wrapperClass)[0]; if (main) { var hamburgerLeft = $(".main-menu-toggle.left-panel .hamburger"); var headroom = new Headroom(main, $.extend({ tolerance: 5, offset: 100, classes: { initial: "animated", pinned: "slideDown", unpinned: "slideUp" }, }, options || {})); headroom.init(); $(main).data("headroom-instance", headroom) } }; var initToggleButtons = function ($) { var hamburgerRight = $(".main-menu-toggle.right-panel .hamburger"); var wrapperRight = $(".side-panel-wrapper.right"); var nag = $(".next-step-nag"); hamburgerRight.on("click", function (event) { hamburgerRight.toggleClass("is-active"); wrapperRight.toggleClass("opened") }); nag.on("click", function (event) { window.location = $(this).find("a").attr("href") }) }; var beforeInitSwup = function ($) { initNextDown($); initOverlayPopup($); initMenuToggle($); $(document).on('flock:sidebar:opened', function () { setupSticky($) }); $(document).on('flock:sidebar:closed', function () { destroySticky($) }) }
var onLoadHook = function ($, initial) {
  initOverlayed($); replaceVars($); setTimeout(function () { initSidebar($) }, 200); initPdfs($); initSlides($); initNextStep($); initSectionFirstSteps($); initSectionVideoAndBanner($); initSectionSocialNetworksShare($); initFlockSlide($); initContactForm($, '.overlay-contact-form', '[data-contact-form-trigger="1"]'); initImpressPresentation($); if (window.twttr) { twttr.widgets.load() }
  if (window.instgrm) { window.instgrm.Embeds.process() }
}
var replaceVarsDecode = function (s) { return decodeURIComponent(s.replace(/\+/g, ' ')) }; var isEmptyObject = function (obj) { return Object.keys(obj).length === 0 && obj.constructor === Object }; var replaceVarsSetCookie = function (value) { return $.pgwCookie({ name: 'flockVars', json: !0, path: '/', secure: window.location.protocol === 'https:', value: value }) }; var replaceVarsGetCookie = function () { return $.pgwCookie({ name: 'flockVars', json: !0, path: '/', secure: window.location.protocol === 'https:' }) }; var replaceVars = function ($) {
  var assocs = {}; var nodes = {}; var queryString = window.location.search.substring(1); var keyValues = queryString.split('&'); for (var i in keyValues) { var key = keyValues[i].split('='); if (key.length > 1) { assocs[replaceVarsDecode(key[0])] = replaceVarsDecode(key[1]) } }
  if (isEmptyObject(assocs)) { var fromCookie = replaceVarsGetCookie(); if (fromCookie) assocs = fromCookie }
  $('[data-fvar]').forEach(function (node) { var $node = $(node); var key = $node.data('fvar'); nodes[key] = nodes[key] || []; nodes[key].push($node) }); for (var key in nodes) { var $nodes = nodes[key]; var value = assocs[key]; $nodes.forEach(function ($node) { var presentValue = $node.data('fpresent'); var missingValue = $node.data('fmissing'); if (value) { if (presentValue && presentValue !== '') { $node.replaceWith(presentValue.replace('__value__', value)) } else { $node.replaceWith(value) } } else if (missingValue && missingValue !== '') { $node.replaceWith(missingValue) } }) }
  if (isEmptyObject(assocs)) { replaceVarsSetCookie(null) } else { replaceVarsSetCookie(assocs) }
}
var swup; var initSwup = function ($) {
  swup = new Swup({
    skipPopStateHandling: function () {
      var hash = window.location.hash; if (hash !== '' && hash.indexOf('#/step-') === 0) { return !0 }
      return !1
    }, LINK_SELECTOR: 'a[href^="/"]:not([data-no-swup]), a[href^="#"]:not([data-no-swup]), a[xlink\\:href], body:not(.impress-path) .next-step a',
  }); swup.usePlugin(swupMergeHeadPlugin, { runScripts: !0 }); function initConsentBanner() {
    const cookiesAccepted = localStorage.getItem('cookies_accepted'); if (!cookiesAccepted) { document.getElementById('cookie-consent').style.display = 'block' } else { document.getElementById('cookie-consent').style.display = 'none' }
    const acceptButton = document.getElementById('accept-cookies'); if (acceptButton) { acceptButton.addEventListener('click', function () { document.getElementById('cookie-consent').style.display = 'none'; localStorage.setItem('cookies_accepted', 'true') }) } else { console.error('Accept button not found') }
  }
  $(document).ready(initConsentBanner); document.addEventListener('swup:contentReplaced', function () { initConsentBanner() }); document.addEventListener('swup:willReplaceContent', function (event) { var $sectionImpress = $('.section-embedded-impress'); if ($sectionImpress.length > 0) { $('.next-step .toggle a').off(nextStepClickedForImpress); impress().tear() } }); document.addEventListener('swup:contentReplaced', function (event) { var page = swup.cache.getPage(window.location.pathname + window.location.search); var $meta = $(page.originalContent).filter('meta[http-equiv="refresh"]'); if ($meta.length > 0) { var content = $meta.prop('content'); var url = content.replace('0; url=', ''); window.location = url } }); document.addEventListener('swup:pageView', function (event) {
    if (window.dataLayer) { window.dataLayer.push({ 'event': 'VirtualPageview', 'virtualPageURL': window.location.pathname, 'virtualPageTitle': document.title }) }
    if ('function' === typeof window.ga) { window.ga('set', 'page', window.location.pathname); window.ga('send', 'pageview') }
    onLoadHook($); setupYoutubeVideos($); var $site = $('.site'); if ($site.length !== 0) { var forkEndpoint = $site.data('fork-endpoint'); var forkId = $site.data('fork-id'); var preview = $site.data('fork-preview') == '1'; if ('function' === typeof window._flock_forks) { window._flock_forks('replace', forkEndpoint, forkId, preview) } }
  })
}
var flockSlideSimpleItemsAnimate = function ($, $slide, $header, $secondaryHeader, $items) { var itemDuration = 500; $items.each(function (index, item) { var $item = $(item); $item.velocity({ opacity: 1, scaleX: 1.2, scaleY: 1.2, }, { duration: itemDuration, delay: (index * itemDuration), }).velocity({ scaleX: 1, scaleY: 1 }) }); if (isTouchDevice()) { $items.velocity({ scaleX: 1 }, { complete: function () { $slide.css('overflow-y', 'auto') } }) } }; var flockSlideBulletListAfterHeaderAnimate = function ($, $slide, $header, $secondaryHeader, $items) { if ($secondaryHeader.length) { $secondaryHeader.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 1, }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1, }, { complete: function (elements) { flockSlideSimpleItemsAnimate($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideSimpleItemsAnimate($, $slide, $header, $secondaryHeader, $items) } }; var flockSlideBulletListAnimate = function ($, $slide, $header, $secondaryHeader, $items) { if ($header.length > 0) { $header.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 1, }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1, }, { complete: function (elements) { flockSlideBulletListAfterHeaderAnimate($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideBulletListAfterHeaderAnimate($, $slide, $header, $secondaryHeader, $items) } }; var flockSlideLineDotsAfterHeaderAnimate = function ($, $slide, $header, $secondaryHeader, $items) { if ($secondaryHeader.length) { $secondaryHeader.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 1, }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1, }, { complete: function (elements) { flockSlideSimpleItemsAnimate($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideSimpleItemsAnimate($, $slide, $header, $secondaryHeader, $items) } }; var flockSlideLineDotsAnimate = function ($, $slide, $header, $secondaryHeader, $items) { if ($header.length > 0) { $header.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 1, }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1, }, { complete: function (elements) { flockSlideLineDotsAfterHeaderAnimate($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideLineDotsAfterHeaderAnimate($, $slide, $header, $secondaryHeader, $items) } }; var flockSlideVennItemsAnimate = function ($, $slide, $header, $secondaryHeader, $items) { var itemDuration = 500; $items.each(function (index, item) { var $item = $(item); var $itemIcon = $item.find('.item-icon'); $itemIcon.css({ opacity: 0 }); $item.velocity({ translateZ: 0, scaleX: 1, scaleY: 1, opacity: 1, }, { duration: itemDuration, delay: (index * itemDuration), }); $itemIcon.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 0.7, }, { duration: itemDuration, delay: (index * itemDuration), }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1 }) }) }
var flockSlideVennAfterHeaderAnimate = function ($, $slide, $header, $secondaryHeader, $items) { if ($secondaryHeader.length) { $secondaryHeader.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 1, }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1, }, { complete: function (elements) { flockSlideVennItemsAnimate($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideVennItemsAnimate($, $slide, $header, $secondaryHeader, $items) } }
var flockSlideVennAnimate = function ($, $slide, $header, $secondaryHeader, $items) { if ($header.length > 0) { $header.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 1, }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1, }, { complete: function (elements) { flockSlideVennAfterHeaderAnimate($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideVennAfterHeaderAnimate($, $slide, $header, $secondaryHeader, $items) } }; var flockSlideRectangleAfterHeaderAnimate = function ($, $slide, $header, $secondaryHeader, $items) { if ($secondaryHeader.length) { $secondaryHeader.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 1, }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1, }, { complete: function (elements) { flockSlideSimpleItemsAnimate($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideSimpleItemsAnimate($, $slide, $header, $secondaryHeader, $items) } }
var flockSlideRectangleAnimate = function ($, $slide, $header, $secondaryHeader, $items) { if ($header.length > 0) { $header.velocity({ translateZ: 0, scaleX: 1.2, scaleY: 1.2, opacity: 1, }).velocity({ translateZ: 0, scaleX: 1, scaleY: 1, }, { complete: function (elements) { flockSlideRectangleAfterHeaderAnimate($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideRectangleAfterHeaderAnimate($, $slide, $header, $secondaryHeader, $items) } }; var flockSlideStartAnimation = function ($, $slide, $header, $secondaryHeader, $items) { if ($slide.hasClass('slide-bullet-list') || $slide.hasClass('slide-bullet-list-numbered')) { flockSlideBulletListAnimate($, $slide, $header, $secondaryHeader, $items) } else if ($slide.hasClass('slide-line-dots') || $slide.hasClass('slide-line-dots-dispatch')) { flockSlideLineDotsAnimate($, $slide, $header, $secondaryHeader, $items) } else if ($slide.hasClass('slide-venn')) { flockSlideVennAnimate($, $slide, $header, $secondaryHeader, $items) } else if ($slide.hasClass('slide-rectangle')) { flockSlideRectangleAnimate($, $slide, $header, $secondaryHeader, $items) } }
var flockSlideHideState = function ($, $node) { $node.velocity({ translateZ: 0, scaleX: 0, scaleY: 0, opacity: 0, }, { duration: 0 }) }
var initFlockSlide = function ($) { var impressRoot = document.getElementById('impress'); $('.section-flock-slide').forEach(function (slide) { var $slide = $(slide); var $header = $slide.find('.header'); var $secondaryHeader = $slide.find('.secondary-header'); var $items = $slide.find('.content .item'); if (impressRoot && $slide.parents('.section-embedded-impress').length !== 0) { impressRoot.addEventListener('impress:stepleave', function (event) { if ($slide.find('.substep').length == 0) { flockSlideHideState($, $header); flockSlideHideState($, $secondaryHeader); flockSlideHideState($, $items) } }); impressRoot.addEventListener('impress:stepenter', function (event) { var $currentStep = $(event.target); $currentStep.scrollTop(0); if ($.contains($currentStep[0], $slide[0]) && $slide.find('.substep').length == 0) { flockSlideHideState($, $header); flockSlideHideState($, $secondaryHeader); flockSlideHideState($, $items); flockSlideStartAnimation($, $slide, $header, $secondaryHeader, $items) } }) } else { flockSlideHideState($, $header); flockSlideHideState($, $secondaryHeader); flockSlideHideState($, $items); flockSlideStartAnimation($, $slide, $header, $secondaryHeader, $items) } }) }; var initSectionFirstSteps = function ($) { $('.section-first-steps .step').forEach(function (step) { var $step = $(step); var $extraContent = $step.find('.extra-content .wrapper'); if ($extraContent.length > 0) { tippy.one(step, { html: $extraContent[0], arrow: !0, size: 'large', performance: !0, inertia: [600, 300], interactive: !0, }) } }) }
var initSectionSocialNetworksShare = function ($) { $('.section-social-networks-share [title]').forEach(function (el) { tippy.one(el, { arrow: !0, performance: !0, inertia: [600, 300], interactive: !0, }) }) }
var nextStepClickedForImpress = function (event) { window.impress().next(); event.preventDefault(); return !1 }
var initImpressPresentation = function ($) {
  var $impressSection = $('.section-embedded-impress'); if ($impressSection.length > 0) {
    document.addEventListener('impress:init', function (event) {
      var api = event.detail.api; var $root = $(event.target); $root.find('[data-goto-next]').forEach(function (next) { var $next = $(next); $next.on('click', function (ev) { var targetStep = $next.data('goto-next'); api.goto(targetStep); return !1 }) }); $('.next-step a').click(nextStepClickedForImpress); if (isTouchDevice()) { api.lib.util.triggerEvent(document, "impress:help:add", { command: "Swipe Left &amp; Right", text: "Previous &amp; Next step", row: 1 }) } else { api.lib.util.triggerEvent(document, "impress:help:add", { command: "H", text: "Show this help", row: 0 }); api.lib.util.triggerEvent(document, "impress:help:add", { command: "Keyboard arrow keys", text: "Previous &amp; Next step", row: 1 }) }
      $root[0].addEventListener('impress:stepenter', function (event) { var $currentStep = $(event.target); var sectionId = $currentStep.data('sec-id'); _flockj_fctxs[0].data.sec = sectionId; if (_flockj && sectionId) _flockj('trackPageView', null, _flockj_fctxs); })
    }); impress().init()
  }
}
var initSectionVideoAndBanner = function ($) {
  var $videos = $('.section-video-and-banner .video')
  var $overlays = $videos.children('.overlay'); $videos.each(function () { var $video = $(this); var $overlay = $video.children('.overlay'); $overlay.on('click', function (e) { $overlays.show(); $videos.removeClass('selected'); $videos.forEach(function (v) { if (v !== $video[0]) { if (v && v.player) { v.player.stopVideo() } else { var videoPlayer = $(v).children('video')[0]; if (videoPlayer) { videoPlayer.pause(); videoPlayer.controls = !1 } } } }); $overlay.hide(); $video.addClass('selected'); if ($video[0] && $video[0].player) { $video[0].player.playVideo() } else { var videoPlayer = $video.children('video')[0]; if (videoPlayer) { videoPlayer.play(); videoPlayer.controls = !0 } } }) })
}
var initMenuToggle = function ($) { $(document).on('click', '.top-menu .toggle-sidebar span', function (ev) { var $site = $('.site'); var $button = $(ev.target); ev.preventDefault(); $site.toggleClass('nav-open'); $button.toggleClass('is-active'); if ($site.hasClass('nav-open')) { $(document).trigger('flock:sidebar:opened') } else { $(document).trigger('flock:sidebar:closed') } }) }; var stickySidebar = null; var setupSticky = function ($) {
  if ($(window).width() > 768) {
    var bottomSpacing = 20; var topSpacing = 0; if ($('.path-steps-menu .banner').length > 0) { topSpacing = 65 }
    if ($(document.body).hasClass('flock-overlayed')) { topSpacing = 0 }
    if (!stickySidebar) { stickySidebar = new StickySidebar('.path-steps-menu', { bottomSpacing: bottomSpacing, containerSelector: '.path-steps-container', innerWrapperSelector: '.path-steps-menu-inner' }) } else { destroySticky($); setupSticky($) }
  }
}; var destroySticky = function ($) { if (stickySidebar) { stickySidebar.destroy(); stickySidebar = null } }; var initOverlayed = function ($) {
  var queryString = window.location.search; if (queryString.indexOf('flock-overlay=1') !== -1) {
    $(document.body).addClass('flock-overlayed'); $('a[href]').forEach(function (a) {
      if (!$('a').data('overlay-popup')) {
        var href = a.href; if (href.indexOf('?') !== -1) { href = href + '&flock-overlay=1' } else { href = href + '?flock-overlay=1' }
        a.href = href
      }
    })
  }
  ...
  $(document).on('click', 'a[href][data-overlay-popup]', function (ev) {
    var link = $(ev.target).closest('a[data-overlay-popup]'); var src = link.prop('href'); if (src.indexOf('/') === 0) { src = '://' + window.location.host + src }
    if (src.indexOf('?') !== -1) { src = src + '&flock-overlay=1' } else { src = src + '?flock-overlay=1' }
    picoModal({ content: '<div class="loading">Loading...</div><iframe src="' + src + '" style="border:0;width:100%;height:100%;"></iframe>', modalStyles: { left: '1.5vw', top: '1.5vh', width: '97vw', height: '97vh', maxHeight: '97vh', boxShadow: 'rgba(0, 0, 0, 0.5) 0 0 10px', }, closeStyles: function (styles) { styles.top = '-7px'; styles.right = '-7px'; styles.zIndex = 2; return styles } }).afterShow(function (modal) {
      var $modal = $(modal.modalElem())
      var $overlay = $(modal.overlayElem())
      var $iframe = $(modal.modalElem()).find('iframe'); var $loading = $(modal.modalElem()).find('.loading'); $loading.css({ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', zIndex: 0, }); $overlay.css('opacity', 0); $iframe.css({ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, opacity: 0, zIndex: 1 }); $modal.velocity({ opacity: 0, translateY: '100vh' }, { duration: 0 }); $overlay.velocity({ opacity: 0.5 }, { duration: 500 }); $modal.velocity({ opacity: 1, translateY: 0 }, { easing: [.42, 0, .58, 1], duration: 500, }); $iframe.on('load', function (ev) { $iframe.css('background-color', '#fff'); $iframe.velocity({ 'opacity': 1 }, { easing: [.42, 0, .58, 1], duration: 500, }) })
    }).beforeClose(function (modal, event) { event.preventDefault(); $(modal.modalElem()).velocity({ opacity: 0, translateY: '100vh' }, { easing: [.42, 0, .58, 1], duration: 500, complete: modal.forceClose }) }).afterClose(function (modal) { modal.destroy() }).show(); ev.preventDefault()
  })
}
var initSidebar = function ($) { var $sidebar = $('.path-steps-menu'); if ($sidebar.length > 0 && $('.site').hasClass('nav-open')) { setupSticky($) } }; var initPdfs = function ($) { var newHost = '//' + window.location.host + '/_files/'; $('[data-pdf]').forEach(function (pdf) { var $pdf = $(pdf); var url = $pdf.data('pdf').replace(/https?:\/\/[^/]+\/?/, newHost); PDFObject.embed(url, $pdf[0], { PDFJS_URL: "/pdfjs/web/viewer.html" }) }) }; var initSlides = function ($) { $('.section-embedded-slides .swiper-container').forEach(function (slide) { var swiper = new Swiper(slide, { pagination: { el: '.swiper-pagination', type: 'bullets', clickable: !0 }, autoHeight: !0, grabCursor: !0, navigation: { nextEl: '.swiper-button-next', prevEl: '.swiper-button-prev', }, slidesPerView: 1, paginationClickable: !0, spaceBetween: 30, loop: !1, }) }) }; var initNextStep = function ($) { $('[data-next-step]').forEach(function (nextStep) { var $nextStep = $(nextStep); $nextStep.on('click', '.toggle', function (ev) { $nextStep.toggleClass('opened'); ev.preventDefault(); return !1 }) }) }; var initNextDown = function ($) { $(document).on('click', '[data-next-down]', function (ev) { var current = $(ev.target).parents('section'); var nextSection = current.next('section'); ev.preventDefault(); nextSection.velocity('scroll', { duration: 1500, easing: [.42, 0, .58, 1] }) }) }; var setupYoutubeVideos = function ($) { $('.section-video-and-banner .video.youtube').forEach(function (video) { var $video = $(video); var $forIframe = $video.children('.for-iframe'); var url = $(video).data('video-url') || ''; var splitted = url.split('?')[0].split('/'); var id = splitted[splitted.length - 1]; $video[0].player = new YT.Player($forIframe[0], { height: '100%', width: '100%', videoId: id, playerVars: { autoplay: 0, controls: 0, modestbranding: 1, rel: 0, showinfo: 0 }, }) }) }
var initYoutubeAPI = function ($) { var tag = document.createElement('script'); tag.src = "https://www.youtube.com/iframe_api"; var firstScriptTag = document.getElementsByTagName('script')[0]; firstScriptTag.parentNode.insertBefore(tag, firstScriptTag) }
var onYouTubeIframeAPIReady = function () { setupYoutubeVideos(Zepto) }
Zepto(function ($) { beforeInitSwup($); initSwup($); initYoutubeAPI($); onLoadHook($); initHeadroom($, ".next-step-nag", { offset: 300, classes: {} }); initToggleButtons($) })